...
export function FunnelPage({ data, view, funnelMode, setFunnelMode, gsStatus }: FunnelPageProps) {
        ...
        {(['planning', 'actuals'] as FunnelMode[]).map((m) => (
          <button
            ...
            className={`px-3.5 py-1.5 rounded-sm text-xs cursor-pointer border-none font-sans transition-all duration-150 ${
              funnelMode === m
                ? 'bg-white text-text font-medium shadow-card'
  ...
  const bClass = am != null
    ? (am / target >= 0.9 ? 'bg-green-bg border border-green-border' : am / target >= 0.7 ? 'bg-amber-bg border border-[#F5C97A]' : 'bg-red-bg border border-[#F09595]')
    : 'bg-amber-bg border border-[#F5C97A]';
  ...
  const stateRows = (['rj', 'od', 'jh'] as const).map((st) => {
    ...
    const conv = a.leads && a.migrated ? ((Number(a.migrated) / Number(a.leads)) * 100).toFixed(1) + '%' : '—';
    ...
    const pc = a.migrated != null
      ? (Number(a.migrated) >= tgt ? 'pill-good' : Number(a.migrated) >= tgt * 0.8 ? 'pill-warn' : 'pill-bad')
      : 'pill-na';
    return (
      <tr key={st}>
        ...
        <td className="text-right"><span className={`pill ${pc}`}>{pct}</span></td>
        <td className="text-right text-text-2 text-[11px]">{gap != null ? (gap <= 0 ? '✓ on track' : `−${gap} short`) : '—'}</td>
  ...
  return (
    <div className="card">
      ...
      <div className={`rounded-lg p-4 px-5 flex gap-6 flex-wrap items-center mb-4 ${bClass}`}>
        <div><div className="text-2xl font-semibold tracking-tight">{am != null ? Number(am).toLocaleString() : '—'}</div><div className="text-[11px] text-text-2 mt-0.5">Actual migrations</div></div>
        <div><div className="text-2xl font-semibold tracking-tight">{target}</div><div className="text-[11px] text-text-2 mt-0.5">Programme target</div></div>
        <div><div className="text-2xl font-semibold tracking-tight">{am != null ? Math.round(am / target * 100) + '%' : '—'}</div><div className="text-[11px] text-text-2 mt-0.5">% of target</div></div>
        <div><div className="text-2xl font-semibold tracking-tight">{oac ? oac + '%' : '—'}</div><div className="text-[11px] text-text-2 mt-0.5">Lead → join conv.</div></div>
      ...
      <div className="mt-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-xs">
            <thead>
              <tr>
                {['State', 'Leads', 'Migrated', 'Target', '% Target', 'Gap', 'Conv.', 'Updated'].map((h, i) => (
                  <th key={h} className={`text-left p-1.5 px-2.5 text-[10px] font-semibold tracking-[0.05em] text-text-3 border-b border-border uppercase ${i > 0 ? 'text-right' : ''}`}>
                    {h}
                  </th>
                ))}
              </tr>
